...
/*
  ...
  2. Movimento reduzido e visibilidade: frame único e estático com
     `prefers-reduced-motion`; IntersectionObserver a pausar o loop fora do
     ecrã.
  3. Dependências do efeito: aqui já eram escalares (string/number/boolean),
     por isso já eram estáveis entre renders — confirmado, sem mexer.

  Correção adicional: `generateNoiseTexture(256)` corre um duplo ciclo de
  256×256 com oito oitavas de ruído no thread principal. O resultado não
  depende de nenhuma prop, por isso passa a gerar-se uma vez por módulo em
  vez de uma vez por montagem — ver `getNoiseData` abaixo.
*/
...
/*
  Constante lazy de módulo: as centenas de milhares de operações do duplo
  ciclo só correm quando alguém pede a textura pela primeira vez, e nunca
  mais — mesmo que o componente monte e desmonte várias vezes (por exemplo,
  entre navegações de idioma).
*/
let cachedNoiseData: Uint8Array | null = null
function getNoiseData(): Uint8Array {
  if (!cachedNoiseData) cachedNoiseData = generateNoiseTexture(256)
  return cachedNoiseData
}
...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    if (reducedMotion) {
      // Frame único: o olho fica aberto e parado, sem o flicker de ruído a
      // correr para quem pediu menos movimento.
      frame(0)
    } else {
      // Fora do ecrã, o loop pára — esta secção é WebGL a ecrã inteiro, o
      // dobro do custo de o deixar a correr sem ninguém a ver.
