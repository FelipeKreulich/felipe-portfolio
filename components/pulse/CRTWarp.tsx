...
export default function CRTWarp({
  ...
  useEffect(() => {
    ...
    /*
      O original abre o seu próprio `requestAnimationFrame`. Este site tem um
      só, o do ticker do GSAP, e é dele que tudo pende — o Lenis, o
      ScrollTrigger e as animações todas. Um segundo laço significava dois
      relógios a competir pelo mesmo frame.

      O limite de fps mantém-se: o ticker corre a 60, o shader não precisa de
      mais de 30, e saltar frames aqui é metade do custo desta secção.
    */
    const render = () => {
    ...
    return () => {
      ...
      renderer.dispose();
      /*
        `dispose()` larga os recursos mas nem sempre o contexto. O
        `WEBGL_lose_context` força-o, e é isso que faz o contador de
        contextos voltar a dois quando esta secção sai de vista.
      */
