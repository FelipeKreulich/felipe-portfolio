...
import ClickSpark from "@/components/background/ClickSpark"
import {
...
export default function LanguageGate({ current }: { current: Locale }) {
  ...
  /*
    O foco inicial é programático, para o Enter confirmar a opção detetada
    sem obrigar ninguém a navegar. Só depois de o utilizador carregar numa
    tecla é que o foco passa a acender a bandeira — senão a opção
    pré-destacada aparecia já com a imagem, sem ninguém lhe ter tocado.
  */
  ...
  */
  ...
  return (
    ...
    <div
      ...
      onPointerMove={() => keyboard && setKeyboard(false)}
      ...
      */}
      <div className="flex items-stretch gap-12 [&:has(button:hover)_button:not(:hover)]:opacity-40 [&:has(button:hover)_button:not(:hover)]:saturate-50 sm:gap-28">
        {locales.map((locale, index) => {
          ...
          return (
            <Magnet key={locale} padding={40} magnetStrength={14} innerRatio={1.8}>
            <button
              ref={(node) => {
            ...
            >
              {/*
                A bandeira entra no hover e sai devagar — 250ms a entrar,
                650ms a sair, para o desaparecimento se ler como recuo e não
                como corte.
              */}
              <img
                ...
                aria-hidden
                /*
                  Só no hover. O foco NÃO acende a bandeira: a opção detetada
                  recebe foco inicial para o Enter a confirmar, e isso fazia a
                  imagem aparecer sozinha. Quem marca o foco são as esquadrias.
                */
                className={`pointer-events-none absolute inset-0 h-full w-full object-cover opacity-0 transition-opacity duration-[650ms] ease-out group-hover:opacity-30 group-hover:duration-[250ms] ${
                  keyboard ? "group-focus-visible:opacity-30 group-focus-visible:duration-[250ms]" : ""
                }`}
              />
    ...
    {/*
      As partículas vivem só neste ecrã. Irmão e não filho: dentro do bloco
      acima herdariam o fade de saída, e queremos que o último spark termine
      de forma limpa.
    */}
    <ClickSpark />
    </>
